...
class T2TAttention(nn.Module):
        ...
        if attn_cfg is None:
            ...
            try:
                if attn_cfg['name'] == 'full':
                    ...
                    self.attention_layer = KDEformer(dim=64, sample_size=sample_size, num_projs=7, Bucket_size=bucket_size, softmax_temp=self.scale)
    ...
    def forward(self, x):
        ...
        if self.attention_layer is None:  # Full attention
            ...
            if self.attention_layer.__class__.__name__ in ["HyperAttention", "KDEformer"]:
